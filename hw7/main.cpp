...
int main()
{
    ...
    vector<int> cards[num_friends][num_vals][num_colors];
    ...
    for (int i = 0; i < num_friends; i++)
    {
      ...
      while (cin.peek() != '\n' && cin.peek() != -1)
      {
        ...
        cards[i][x-1][c-1].push_back(cur_vert_index);
        cur_vert_index += 2;
        num_vertices += 2;
      }
      cin.get();
    ...
    for(int f = 0; f < num_friends; f++) {
        for(int v = 0; v < num_vals; v++) {
            for(int c = 0; c < num_colors; c++) {
                if(!cards[f][v][c].empty()) {

                    cout << "Checking f: " << f << ", v: " << v << ", c: " << c << endl;

                    for(int card_addr : cards[f][v][c]) {
                        cur_hd = card_addr;
                        cur_tl = card_addr + 1;

                        graph[cur_hd].push_back({cur_tl, edge_index++});
                        graph[cur_tl].push_back({cur_hd, edge_index++});
                        cap.push_back(1);
                        cap.push_back(0);

                        next_player = (f + 1) % num_friends;

                        for(int any_color = 0; any_color < num_colors; any_color++) {
                            for(int target_card : cards[next_player][v][any_color]) {
                                edge_hd = cur_tl;
                                edge_tl = target_card;
                        ...
                        if(v != num_colors - 1) {
                            for(int target_card : cards[next_player][v+1][c]) {
                                edge_hd = cur_tl;
                                edge_tl = target_card;
    ...
    for(int f = 0; f < num_friends; f++) {
        for(int c = 0; c < num_colors; c++) {
            for(int target_card : cards[f][0][c]) {
                edge_hd = s;
                edge_tl = target_card;
            ...
            for(int target_card : cards[f][num_vals-1][c]) {
                edge_hd = target_card + 1;
                edge_tl = t;
    ...
    cout << "s: " << s << ", t: " << t << ", verts: " << num_vertices << endl;
